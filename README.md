...
20.6. [Phase 6-8 開發藍圖（全面升級）](#206-phase-6-8-開發藍圖全面升級)
20.7. [研究參考文獻（Phase 6-8）](#207-研究參考文獻phase-6-8)
...
# 20.6 Phase 6-8 開發藍圖（全面升級）

> **定位**：超越原有 Phase 1-4 框架，以用戶需求為核心，從「基本可用」升級至「令人上癮的學習體驗」
> **適用版本**：Math RPG v2.0+
...
| 需求 | 說明 | 對應系統 |
|------|------|----------|
| **題目分年級** | 題目按 S1-S6 年級分類，老師易於編輯 | 題庫管理系統 |
  > **研究參考**：題庫設計參考 [OpenAI grade-school-math](https://github.com/openai/grade-school-math)（8,500+ 按年級分段題目）及 [arXiv Data×LLM](https://arxiv.org/html/2505.18458v2)（JSON Schema 標準格式）
  >
  > **推薦 JSON Schema（適用於題庫重構）**：
  > ```json
  > {
  >   "id": "alg_s1_001",
  >   "topic": "代數",
  ...
  >   "answer": "A",
  >   "hints": ["hint1", "hint2"],
  >   "commonMistakes": ["A同學常錯...", "B同學常錯..."]
  > }
  > ```
| **場館系統** | 每個小遊戲等於一個挑戰場館 | 地圖 2.0 + 場館大廳 |
| **道場挑戰** | S4 學生可以體驗 S1 能力水平，彌補實際年級與展現能力之間的差距 | 道場跨年級系統 |
| **老師管主** | 老師作為場館管理者，可為學生創作自定義題目 | 老師後台 |
...
| 支柱 | 核心概念 | 關鍵功能 |
|------|----------|----------|
| 🗺️ **探險** | 學校地圖 + 迷霧系統 + 傳送門 + 隨機事件 | 地圖 2.0、傳送門、隨機事件 |
| 📈 **個人成長** | 技能樹 + 五大職業 + 稱號系統 | 計算師/幾何師/代數師/統計師/應用師 |
| 🏆 **勝負慾** | 班級排名 + 每週挑戰 + 個人最佳 + 挑戰信 | 排行榜系統、挑戰信 |
| 🎉 **成功感** | 煙火升級 + 成就彈窗 + 收集進度條 + 里程碑慶祝 | 特效增強、戰績牆 |
| ❓ **好奇心** | 神秘 NPC + 鎖定區域提示 + 每日謎題 + 隱藏成就 + 卡牌故事 | 謎題系統、隱藏內容 |

---

## Phase 6：即時滿足（🚀 立即可基於現有代碼實現）

> **目標**：不做新功能，只做「已有數據的更好呈現」
> **預計工時**：2-3 週
> **技術難度**：低（純前端 UI 改動）
...
|------|------|
| **顯示位置** | 角色頭頂，等級旁邊 |
| **稱號來源** | 等級決定、成就解鎖、職業赋予、特殊條件 |
| **示例** | 「見習魔法師」(Lv.1-10) / 「數學新星」(答對100題) / 「計算師」(職業) |
| **技術方案** | 在角色精靈上方 render 文字標籤，讀取現有的 `player.title` 欄位 |
...
|------|------|
| **入口** | 個人資料頁 → 「戰績」按鈕 |
| **顯示內容** | 總答題數 / 總正確數 / 正確率 / 最長連勝 / 等級 / 職業 / 總遊戲時間 |
| **圖表** | 正確率趨勢圖（近30天）、各課題正確率餅圖 |
| **技術方案** | 讀取 Firebase 現有數據，用 Chart.js（CDN）渲染圖表，無需後端改動 |
...
| 項目 | 說明 |
|------|------|
| **現有** | 簡單粒子煙火（Phase 1c 已實現） |
| **增強** | 等級段位特效（每10級不同）、成就解鎖彈窗、個人里程碑慶祝 |
| **特效清單** | 等級提升時播放對應等級段位的慶祝動畫；首次解鎖成就時全屏公告 |
| **技術方案** | 在 `levelUp()` 函數中加入 `playLevelUpEffect(level)` 分支，調用 Canvas particle system |

**等級段位特效：**

| 等級段 | 特效 |
|--------|------|
| Lv. 10/20/30/40/50 | 🏆 金色煙火 + 大型公告 |
| Lv. 11-19 | 銀色煙火 + 小型公告 |
| 其他 | 基礎煙火（現有） |

---

### 6.4 📈 收集進度條（在現有卡牌計數上加 UI）

| 項目 | 說明 |
|------|------|
| **位置** | 卡牌收藏頁頂部 |
| **顯示** | 普通卡 X/20（60%）｜稀有卡 X/20（30%）｜傳說卡 X/10（10%） |
| **技術方案** | 讀取 `player.cards[]`，按稀有度分組，計算完成度，render progress bar |

---

## Phase 7：持續動力（📋 第二波）

> **目標**：加入時間維度，讓玩家每天都有理由回來
> **預計工時**：4-6 週
> **技術難度**：中（需要定時結算系統）
...
| 項目 | 說明 |
|------|------|
| **機制** | 每週一 00:00 自動發布，挑戰內容拷貝每日挑戰 + 額外結算 |
| **結算** | 每週日 23:59 結算，TOP 10 獲得榮譽點 + 特殊稱號 |
| **榜單** | 每週排行榜（獨立於總排行榜） |
| **示例挑戰** | 「本週答對 50 題」「本週總經驗 TOP 50」 |

**每週挑戰示例：**

| 挑戰名 | 條件 | 獎勵 |
|--------|------|------|
| 週冠之路 | 本週答對 ≥50 題 | 200 學分 |
| 幾何之週 | 本週幾何題答對 ≥20 題 | 稀有卡牌碎片 x1 |
| 早起鳥 | 本週有3天在 7:00 前答題 | 「早起鳥」稱號 |
| 連勝高手 | 本週最長連勝 ≥10 | 100 榮譽 |
...
| 項目 | 說明 |
|------|------|
| **觸發** | 在地圖上隨機遇到神秘 NPC（每日最多 1 次） |
| **謎題內容** | 不是數學題，而是邏輯謎題、數獨、圖形推理等 |
| **獎勵** | 學分 + 稀有碎片 + 隱藏卡牌 |
| **提示** | 回答錯誤可以看提示（扣部分獎勵） |
| **技術方案** | 在地圖 random event 系統中加入 NPC encounter，調用獨立的謎題題庫 |
...
| **時間維度** | 本週 / 上週 / 歷史最高 |
| **類別** | 等級榜、答題榜、連勝榜、班級榜 |
| **獎勵** | 每週 TOP 10 獲得榮譽積分 + 特殊標識（如 🏅本週之星） |
| **技術方案** | Firebase 添加 `weeklyLeaderboard/{weekId}/{uid}` 結構，每週 GAS cron job 結算 |

---

### 7.4 📦 場館系統（基礎版）

> **研究參考**：根據 [Edutopia 研究](https://www.edutopia.org/article/game-based-learning-middle-)，遊戲化學習能顯著提升學生持續參與度（persistence）。場館系統的「挑戰×即時反饋」循環與此發現一致，建議加入視覺進度條和段位晉升以維持動機。

| 項目 | 說明 |
|------|------|
| **概念** | 每個小遊戲等於一個「場館」，學生進入場館挑戰 |
| **場館類型** | 計算道場（心算）/ 幾何殿堂（空間題）/ 統計堡壘（數據題）/ 應用迷宮（應用題） |
| **老師創作** | 老師可在後台為特定場館添加自定義題目 |
| **技術方案** | 在現有教室系統上包裝成「場館」UI，每個場館有自己的題庫分支 |

---

### 7.5 📝 道場挑戰（跨年級能力體驗）

> **研究參考**：「道場挑戰」的核心價值在於彌合「就讀年級」與「實際展現能力」之間的差距。根據元認知理論，學生需要機會在「安全環境」中重新接觸基礎概念；遊戲化環境中的補救訓練可提升參與度和自信心。設計時以「建議年級」標示而非「強制年級」，尊重學生自我效能感。[^1]

[^1]: 此設計原則融合了 ZPD（最近發展區）理論與遊戲化學習研究（詳見 Section 20.7.5）

| 項目 | 說明 |
|------|------|
| **概念** | S4 學生可以進入 S1-S3 難度的道場，體驗低年級挑戰 |
| **用途** | 程度較弱的學生可以鞏固基礎；老師可指定復健訓練 |
| **難度標示** | 每個道場顯示「建議年級」而非「必須年級」 |
| **獎勵** | 跨年級挑戰成功，經驗加成 x1.5 |
| **技術方案** | 在題庫中加入 `gradeLevel` 欄位，道場挑戰時不受年級限制，純粹按難度匹配 |

---

## Phase 8：深度沉浸（🔧 主線）

> **目標**：完整的 RPG 體驗，讓探索、成长、社交融為一體
> **預計工時**：6-12 個月（可分多個 sub-phase）
> **技術難度**：高（需要地圖編輯器 + 技能樹系統 + 被動計算框架）

### 8.1 🗺️ 學校地圖 2.0

| 項目 | 說明 |
|------|------|
| **迷霧系統** | 未探索區域有迷霧覆蓋，探索後顯現 |
| **傳送門** | 各場館之間的快速傳送節點，需滿足條件解鎖 |
| **隨機事件** | 地圖上定時觸發：寶箱、NPC、每日謎題 |
| **地圖編輯器** | 老師可自定義地圖元素位置（Google Sheets 作為配置） |
| **技術方案** | 需要新的地圖渲染系統（Tile map 或自定義 Canvas），定義地圖數據 JSON 結構 |

**地圖數據結構：**
```javascript
const mapData = {
  tiles: [...], // 32x32 瓷磚網格
  portals: [{from: [x,y], to: [x,y], unlockLevel: 10}],
  npcs: [{id: 'npc_001', x: 5, y: 3, type: 'merchant'}],
  fog: [{x: 0, y: 0, explored: false}, ...],
  events: [{trigger: 'random', type: 'chest', x: 10, y: 10}]
};
```
...
> **研究參考**：技能樹設計原則來源於 [Keys to Meaningful Skill Trees (gdkeys.com)](https://gdkeys.com/keys-to-meaningful-skill-trees/)（有意義的選擇、長期承諾、視覺化進度）以及 [gamedesigning.org Skill Trees](https://gamedesigning.org/learn/skill-trees/)（三叉戟型結構、前置技能機制）。

| 項目 | 說明 |
|------|------|
| **五大職業** | 計算師 / 幾何師 / 代數師 / 統計師 / 應用師 |
| **技能樹結構** | 每個職業三系：攻擊系 / 防禦系 / 輔助系 |
| **被動計算** | 技能效果直接影響答題經驗、傷害計算、掉落率 |
| **UI** | 彈性技能樹面板（三叉戟形），用 Canvas/SVG render |
| **技術方案** | 在玩家數據中添加 `skills: {}` 對象，前端計算最終加成 |
...
[基礎] — [連擊 I] — [連擊 II]
   |
[暴擊 I] — [暴擊 II] — [暴擊 Master]
   |
[速度 I] — [速度 II] — [速度 Master]
```
...
> **研究參考**：老師Dashboard 參考了 [Quizalize](https://www.quizalize.com/en-us)（老師發布→學生競賽模式）及 [QuestionWell](https://app.questionwell.org/)（AI 輔助題目生成）。

| 項目 | 說明 |
|------|------|
| **場館管理** | 老師作為場館館長，可開啟/關閉特定場館 |
| **自定義題目** | 老師可在場館中添加自己的題目（上傳 JSON 或 Google Sheets） |
| **學生視圖** | 老師後台看到全班學生的戰績、弱點課題、學習軌跡 |
| **任務派發** | 老師可向全班/個人派發特定場館挑戰任務 |
| **技術方案** | 老師後台（獨立 HTML 頁面），老師身份由 Firebase Auth 的 `teacher: true` claim 判斷 |
...
| 職業 | 解鎖條件 | 被動效果 |
|------|----------|----------|
| 計算師 | 集齊5張計算相關卡牌 | 算術題經驗 +20% |
| 幾何師 | 集齊5張幾何相關卡牌 | 幾何題經驗 +20% |
| 代數師 | 集齊5張代數相關卡牌 | 代數題經驗 +20% |
| 統計師 | 集齊5張統計相關卡牌 | 統計題經驗 +20% |
| 應用師 | 集齊5張應用相關卡牌 | 應用題經驗 +20% |

> **職業衝突**：玩家同一時間只能選擇一個職業，切换需要冷卻時間（7天）
...
| 項目 | 說明 |
|------|------|
| **卡牌故事** | 每張卡牌有背後故事（數學家生平、公式發現過程等） |
| **查看方式** | 在卡牌詳情頁點擊「📖 故事」按鈕 |
| **隱藏卡牌** | 完成特定條件才顯現（如：集齊某套卡牌） |
| **技術方案** | 在現有卡牌 JSON 中添加 `story` 欄位（長文字），渲染時在 modal 中展示 |
...
| 項目 | 說明 |
|------|------|
| **概念** | 玩家可以向同學發送「挑戰信」，約定挑戰內容 |
| **格式** | 「我想挑戰你！在本週幾何殿堂比試，誰答對更多幾何題誰贏」 |
| **回應** | 被挑戰者可以接受/拒絕 |
| **獎勵** | 挑戰勝出者獲得榮譽 + 特殊稱號 |
...
| 用戶需求 | Phase | 功能 |
|----------|-------|------|
| 題目分年級 | Phase 6 | 現有題庫重構（加 gradeLevel 欄位） |
| 場館系統 | Phase 7 | 教室 → 場館 UI 包裝 |
| 道場挑戰 | Phase 7 | 跨年級題目匹配系統 |
| 老師管主 | Phase 8 | 老師後台 + 自定義題目上傳 |

---

# 20.7 研究參考文獻（Phase 6-8）

本節收錄 Phase 6-8 開發藍圖的理論依據與參考資源，所有文獻均經過網絡研究驗證。

---

## 20.7.1 題庫與年級分類設計

### OpenAI Grade-School Math（GitHub）
- **來源**：[OpenAI/grade-school-math](https://github.com/openai/grade-school-math)
- **內容**：8,500+ 高質量中小學數學題目，按年級分段（Grade 1-8）
- **用途**：題目分年級、難度對齊的實證參考
- **啟示**：題目按年級分類時，建議採用「S1-S6」六級制（對應香港學制），每級 50-100 題標杆題目

### arXiv: Data × LLM — 數學題目存儲格式
- **來源**：[arXiv: Data × LLM for Math](https://arxiv.org/html/2505.18458v2)
- **內容**：CSV、JSON、TSV 格式的數學題目標準化存儲方案
- **用途**：題庫 JSON Schema 設計參考
- **啟示**：推薦採用以下 JSON Schema 儲存題目：
...
```json
{
  ...
  "commonMistakes": [
    "A同學常錯：忽略負號，將 -x + 5y 誤寫成 +3y",
    "B同學常錯：把所有系數都相加，包括x和y的"
  ]
}
```

### Kaggle GSM8K（Grade School Math 8K）
- **來源**：[Kaggle: Grade-School Math 8K](https://www.kaggle.com/datasets/thedevastator/grade-school-math)
- **內容**：8,000 對問答數據集，含自然語言解題過程
- **用途**：Q&A 題型設計、題目自動生成參考

---

## 20.7.2 遊戲化學習場館

### Edutopia — Game-Based Learning in Middle School Math
- **來源**：[Edutopia: Game-Based Learning in Middle School Math](https://www.edutopia.org/article/game-based-learning-middle-)
- **核心發現**：遊戲化學習能顯著提升學生參與度，學生在遊戲環境中的持續性（persistence）明顯高於傳統練習
- **對本項目的啟示**：場館系統的「挑戰×獎勵」循環與 Edutopia 研究結果一致，建議在場館中加入即時反饋、段位晉升和視覺進度條，以維持學生持續挑戰的動機
- **引用重點**：
  > "Students who engaged with game-based learning showed higher persistence rates and were more likely to continue attempting difficult problems."

### ResearchGate — Design of Game-based Learning for Elementary Math
- **來源**：[ResearchGate: Design of Game-based Learning for Elementary Math](https://www.researchgate.net/publication/386261223)
- **核心發現**：小學數學遊戲化學習的設計原則，包括情境化、反饋即時性、難度適配三要素
- **對本項目的啟示**：場館系統須提供「適合學生程度」的挑戰，建議配合「道場挑戰」實現跨年級能力匹配
...
- **來源**：[Keys to Meaningful Skill Trees](https://gdkeys.com/keys-to-meaningful-skill-trees/)
- **核心原則**：
  1. **有意義的選擇**：每個分支必須帶來截然不同的遊戲體驗
  2. **長期承諾**：技能解鎖應具有持久影響，不可逆
  3. **視覺化進度**：技能樹應讓玩家清楚看到已走過的路和即將選擇的方向
- **對本項目的啟示**：計算師/幾何師/代數師/統計師/應用師五大職業的技能樹，應確保各分支帶來明顯不同的被動效果，而非可有可無的「微調」
- **引用重點**：
  > "A good Skill Tree forces the player to make impacting, committing, long-lasting choices for the development of his playstyle."

### Game Designing — Skill Trees
- **來源**：[gamedesigning.org: Skill Trees](https://gamedesigning.org/learn/skill-trees/)
- **核心設計模式**：
  - 三叉戟型結構（攻擊/防禦/輔助三系）
  - 前置技能機制（避免一步到頂）
  - 視覺化連線表示依賴關係
- **對本項目的啟示**：建議在 Phase 8 技能樹中採用「三叉戟型」UI，每個職業有獨立面板，用 Canvas 或 SVG render 技能節點和連線

---

## 20.7.4 老師工具與Dashboard

### Quizalize
- **來源**：[Quizalize](https://www.quizalize.com/en-us)
- **核心功能**：老師創建測驗 → 學生競速答題 → 即時排行榜
- **對本項目的啟示**：老師後台應支援「發布全班挑戰」功能，類似 Quizalize 的發布-競賽模式

### QuestionWell
- **來源**：[QuestionWell](https://app.questionwell.org/)
- **核心功能**：AI 輔助老師自動生成試題
- **對本項目的啟示**：老師後台可整合「AI 生成題目」功能，讓老師輸入課題後自動生成相應難度的題目（Phase 8 擴展功能）
...
### 研究背景
「道場挑戰」的核心價值在於：讓 S4 學生可以體驗 S1-S3 難度的挑戰，彌合「就讀年級」與「實際展現能力」之間的差距。這一設計靈感來自：

1. **元認知理論**：學生需要機會在「安全環境」中重新接觸基礎概念，而不感到「被降級」
2. **ZPD（最近發展區）理論**：學生在「稍微超出當前能力」的難度區間學習效果最佳；過於簡單或過於困難都會降低學習動機
3. **遊戲化康復訓練**：研究顯示，在遊戲環境中進行補救訓練，學生的參與度和自信心均有顯著提升

### 設計建議
- 道場難度以「建議年級」標示，而非「強制年級」——尊重學生的自我效能感
- S4 學生挑戰 S1 道場成功後，給予「鞏固經驗」標籤，而非「降級」標籤
- 跨年級挑戰成功的額外獎勵（x1.5 經驗）可提升動機
...
| # | 來源 | URL | 用於 |
|---|------|-----|------|
| 1 | OpenAI/grade-school-math | https://github.com/openai/grade-school-math | 題庫年級分類 |
| 2 | arXiv: Data × LLM | https://arxiv.org/html/2505.18458v2 | JSON Schema |
| 3 | Kaggle GSM8K | https://www.kaggle.com/datasets/thedevastator/grade-school-math | Q&A 題型 |
| 4 | Edutopia: Game-Based Learning | https://www.edutopia.org/article/game-based-learning-middle- | 場館系統 |
| 5 | ResearchGate: Game-based Math | https://www.researchgate.net/publication/386261223 | 場館設計原則 |
| 6 | Quizalize | https://www.quizalize.com/en-us | 老師Dashboard |
| 7 | QuestionWell | https://app.questionwell.org/ | AI題目生成 |
| 8 | Keys to Meaningful Skill Trees | https://gdkeys.com/keys-to-meaningful-skill-trees/ | 技能樹設計 |
| 9 | gamedesigning.org: Skill Trees | https://gamedesigning.org/learn/skill-trees/ | 技能樹結構 |
